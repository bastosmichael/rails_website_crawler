class Crawler::SitemapperTen < Crawler::Sitemapper
  sidekiq_options queue: :sitemapper_ten,
                  retry: true,
                  backtrace: true,
                  unique: :until_and_while_executing,
                  unique_expiration: 120 * 60

  def perform(url, type = 'ScrimperTen')
    return if url.nil?
    @url = url
    @type = type
  ...
  def get_sitemap(url)
    Crawler::SitemapperTen.perform_async url, @type
  end
end
