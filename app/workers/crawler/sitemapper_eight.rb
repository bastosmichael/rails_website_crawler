class Crawler::SitemapperEight < Crawler::Sitemapper
  sidekiq_options queue: :sitemapper_eight,
                  retry: true,
                  backtrace: true,
                  unique: :until_and_while_executing,
                  unique_expiration: 120 * 60

  def perform(url, type = 'ScrimperEight')
    return if url.nil?
    @url = url
    @type = type
  ...
  def get_sitemap(url)
    Crawler::SitemapperEight.perform_async url, @type
  end
end
