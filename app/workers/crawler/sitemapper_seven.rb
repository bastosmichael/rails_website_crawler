class Crawler::SitemapperSeven < Crawler::Sitemapper
  sidekiq_options queue: :sitemapper_seven,
                  retry: true,
                  backtrace: true,
                  unique: :until_and_while_executing,
                  unique_expiration: 120 * 60

  def perform(url, type = 'ScrimperSeven')
    return if url.nil?
    @url = url
    @type = type
  ...
  def get_sitemap(url)
    Crawler::SitemapperSeven.perform_async url, @type
  end
end
